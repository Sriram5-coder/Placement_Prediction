...
def main():
    ...
    for display_name, column_name in columns.items():
        if column_name in ["thub", "owl_coder", "hero_vired", "both"]:
            ...
            slider_value = st.sidebar.slider(display_name, min_value=0, max_value=100, value=0)
...
#     st.write("Array of values:", values_array)
    ...
    # Extracting the array of companys from the top_companys variable
    companys_array = top_companys[0]

    st.subheader(f"Most Chances to Get Placed In")

    # Iterate over the companys array and display them as list items
    for i, company in enumerate(companys_array):
        rank = i + 1
        if rank == 2:
            st.header("Other companies with similar skill sets and likelihood of placement")
        st.markdown(
