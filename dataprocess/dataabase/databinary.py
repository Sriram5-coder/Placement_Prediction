...
# # Create the table query dynamically using a for loop
# create_table_query = "CREATE TABLE IF NOT EXISTS companyplaced (selected VARCHAR(50), "
...
# alter_query = "ALTER TABLE companyplaced ADD COLUMN company_id INT;"
...
# insert_query = "INSERT INTO companyplaced (selected, company_id) VALUES (%s, %s)"

# # Insert data into the database
# for company_name, company_id in selected.items():
#     cursor.execute(insert_query, (company_name, company_id))
...
# update_query = "UPDATE companyplaced SET company_id = %s WHERE selected = %s"

# # Update company_id based on company_name
# for company_name, company_id in selected.items():
#     cursor.execute(update_query, (company_id, company_name))
...
for i in range(3):
    ...
    for start in eachcom:
        ...
        for labelname in getskills:
            ...
            print(getkey)
            update_query = f"UPDATE companyplaced SET `{getkey}` = 1 WHERE company_id = {companyname}"
            print(update_query)
